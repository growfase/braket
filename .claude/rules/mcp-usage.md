...
| **Supabase** | Supabase project, database, docs, logs, and Edge Function management |
...
## Supabase MCP Usage (direct HTTP)

### Use Supabase for:
1. Listing and querying project database objects
2. Applying tracked database migrations
3. Reading Supabase docs and advisors
4. Inspecting project URLs, publishable keys, logs, and Edge Functions
...
### Authentication:
Run `claude /mcp`, select `supabase`, then authenticate in the browser. The hosted Supabase MCP now uses OAuth by default, so a personal access token is not required for normal local use.
